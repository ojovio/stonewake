...
public class TileMapPhysicsSolver {
    ...
    public void updatePhysics(Camera camera, TileMap tileMap, BitMask bm, World physicsWorld, int layer) {
        ...
        while (it.hasNext()) {
            ...
            if (entryChunkX < minChunkX || entryChunkX > maxChunkX || entryChunkY < minChunkY || entryChunkY > maxChunkY) {
    ...
    public void calculateTilePhysics(BitMask bm, TileMap tileMap, World physicsWorld, int tileLayer, int tileX, int tileY) {
        if (!tileMap.isTileOnBounds(tileLayer, tileX, tileY)) return;

        long key = TileUtils.codifyTilePosition(tileMap, tileLayer, tileX, tileY);
        Tile tile = tileMap.getTile(tileLayer, tileX, tileY);

        if ((tile.isDirtyPhysics() && tileBodies.containsKey(key)) || !tileBodies.containsKey(key)) {
            if (tile.isTileAir()) return;

            int mask = bm.calculateBitMask(tile);
            if (bm.isTileSurrounded(mask)) return;
